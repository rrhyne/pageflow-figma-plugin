    ...
    <div v-if="!settings">
      <div class="flex">
        ...
        <div class="flex-grow text-right" >
          <button @click="toggleSettings()" class='page-button' style="margin-top: -8px;">
      ...
      <div class="flex">
       ...
       <ul class="page-list">
        <li v-for="(page, index) in defaultPages" :key="index">
            <input v-bind:id="'page'+index" type="checkbox" @change="addToSelected($event, page)">
...
export default class PageCreator extends Vue {
  ...
  addToSelected (e, page){
    if(e.target.checked === true) {
      //if it's not added, add
      const pageExists : boolean = (this.selectedDefaults.indexOf(page) === -1) ? false : true
      if(pageExists == false) this.selectedDefaults.push(page)
    }
  }
  ...
  setDefaults(){
    console.log(this.selectedDefaults)
